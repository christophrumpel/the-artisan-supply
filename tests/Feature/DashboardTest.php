...
test('shopkeepers can add a knowledge base entry with an embedding', function () {
    $this->app->instance(KnowledgeBaseEmbeddings::class, new class extends KnowledgeBaseEmbeddings
    {
    ...
    expect(Faq::first())
        ->embedding->toBe([1, 0, 0])
        ->embedding_model->toBe('test-embedding-model');
});

test('knowledge base tool ranks embedded entries by vector similarity', function () {
    Faq::create([
        'question' => 'How do I clean the Queue Worker Lunchbox?',
        'answer' => 'Hand wash the lunchbox only.',
        'embedding' => [0.2, 0.9, 0.0],
    ]);

    Faq::create([
        'question' => 'Can I return the Migration Time Machine?',
        'answer' => 'Returns are available before time travel begins.',
        'embedding' => [0.95, 0.05, 0.0],
    ]);

    $embeddings = new class extends KnowledgeBaseEmbeddings
    {
    ...
    $results = json_decode((string) (new SearchKnowledgeBase($embeddings))->handle(new ToolRequest([
        'query' => 'What is the return policy?',
    ])), true, flags: JSON_THROW_ON_ERROR);

    expect($results)
        ->toHaveCount(1)
        ->and($results[0]['question'])->toBe('Can I return the Migration Time Machine?')
        ->and($results[0]['search_method'])->toBe('vector');
});

test('support reply agent includes knowledge base entries without using the knowledge base tool', function () {
    Faq::create([
        'question' => 'Is the Queue Worker Lunchbox dishwasher safe?',
        'answer' => 'The Queue Worker Lunchbox should be hand washed only.',
    ]);

    $instructions = (new SupportReplyAgent)->instructions();

    expect((string) $instructions)
        ->toContain('Knowledge base:')
        ->toContain('Is the Queue Worker Lunchbox dishwasher safe?')
        ->toContain('The Queue Worker Lunchbox should be hand washed only.')
...
test('customers can submit a voice support message', function () {
    ...
    Transcription::fake(['Can you help me with my order?']);
    ...
    expect($message)
        ...
        ->transcription->toBe('Can you help me with my order?');
